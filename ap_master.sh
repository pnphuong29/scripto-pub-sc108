...
if [ -d "${HOME}/scripto-common/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto-common/libs" -maxdepth 1 -type f -name "ap_servers*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-share/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto-share/libs" -maxdepth 1 -type f -name "ap_servers*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-main/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto-main/libs" -maxdepth 1 -type f -name "ap_servers*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto/libs" -maxdepth 1 -type f -name "ap_env_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-share/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto-share/libs" -maxdepth 1 -type f -name "ap_env_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-common/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto-common/libs" -maxdepth 1 -type f -name "ap_env_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-main/libs" ]; then
    while read -r ap_env; do
        source "${ap_env}"
    done < <(gfind "${HOME}/scripto-main/libs" -maxdepth 1 -type f -name "ap_env_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto/libs" ]; then
    while read -r ap_alias; do
        source "${ap_alias}"
    done < <(gfind "${HOME}/scripto/libs" -maxdepth 1 -type f -name "ap_alias_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto/libs" ]; then
    while read -r ap_func; do
        source "${ap_func}"
    done < <(gfind "${HOME}/scripto/libs" -maxdepth 1 -type f -name "ap_func_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-share/libs" ]; then
    while read -r ap_alias; do
        source "${ap_alias}"
    done < <(gfind "${HOME}/scripto-share/libs" -maxdepth 1 -type f -name "ap_alias_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-common/libs" ]; then
    while read -r ap_alias; do
        source "${ap_alias}"
    done < <(gfind "${HOME}/scripto-common/libs" -maxdepth 1 -type f -name "ap_alias_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-main/libs" ]; then
    while read -r ap_alias; do
        source "${ap_alias}"
    done < <(gfind "${HOME}/scripto-main/libs" -maxdepth 1 -type f -name "ap_alias_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-share/vendors" ]; then
    while read -r ap_vendor; do
        source "${ap_vendor}"
    done < <(gfind "${HOME}/scripto-share/vendors" -maxdepth 1 -type f -name "ap_vendor_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-common/vendors" ]; then
    while read -r ap_vendor; do
        source "${ap_vendor}"
    done < <(gfind "${HOME}/scripto-common/vendors" -maxdepth 1 -type f -name "ap_vendor_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-main/vendors" ]; then
    while read -r ap_vendor; do
        source "${ap_vendor}"
    done < <(gfind "${HOME}/scripto-main/vendors" -maxdepth 1 -type f -name "ap_vendor_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto/vendors" ]; then
    while read -r ap_vendor; do
        source "${ap_vendor}"
    done < <(gfind "${HOME}/scripto/vendors" -maxdepth 1 -type f -name "ap_vendor_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-share/libs" ]; then
    while read -r ap_func; do
        source "${ap_func}"
    done < <(gfind "${HOME}/scripto-share/libs" -maxdepth 1 -type f -name "ap_func_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-common/libs" ]; then
    while read -r ap_func; do
        source "${ap_func}"
    done < <(gfind "${HOME}/scripto-common/libs" -maxdepth 1 -type f -name "ap_func_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
...
if [ -d "${HOME}/scripto-main/libs" ]; then
    while read -r ap_func; do
        source "${ap_func}"
    done < <(gfind "${HOME}/scripto-main/libs" -maxdepth 1 -type f -name "ap_func_*.sh" | grep -v -e "7s" -e "jeditech" | sort)
fi
