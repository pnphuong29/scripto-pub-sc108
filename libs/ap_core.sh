...
# alias setdevicehostname="ap_func_set_device_hostname"
# ap_func_set_device_hostname() {
#     local ap_device_id
#     local ap_device_name
...
#     ap_device_name="$(json5 ~/scripto-common/devices.json | jq -r --arg device_id "${ap_device_id}" '.[] | select(.id == $device_id) | .hostname')"
#     # echo "ap_device_name = [${ap_device_name}]"

#     ap_current_hostname="$(hostname)"
#     # echo "ap_current_hostname = [${ap_current_hostname}]"

#     if [ "${ap_current_hostname}" != "${ap_device_name}" ]; then
#         aplogshow "Set hostname to [${ap_device_name}]\n"
#         sudo hostname "${ap_device_name}"
#     fi
# }
