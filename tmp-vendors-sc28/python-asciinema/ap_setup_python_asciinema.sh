...
if ! type asciinema &>/dev/null; then
	@logshow "Installing [asciinema] using [pip]\n"
	pip3 install asciinema

	if type asciinema &>/dev/null; then
		@logshowpassed "[asciinema] has been installed successfully using [pip]\n"
	else
		@logshowfailed "[asciinema] has been installed unsuccessfully using [pip]\n"
	fi
fi
