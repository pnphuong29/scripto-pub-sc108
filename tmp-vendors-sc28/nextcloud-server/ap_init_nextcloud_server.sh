...
alias bkncdb="cd \${AP_SC28_VAGRANT_VMS_DIR}/vg-nextcloud; vagrant ssh -c /home/vagrant/pnphuong29/projects/p29-github/pnphuong29/ap-scripts-nextcloud-sc56/utils/ap_bk_nextcloud_db.sh"

# Backup Nextcloud source codes, data and DB
alias bkncall="gacpnc7; bkncdata; bkncdb"

# Restore Nextcloud DB from host machine
alias rtncdb="cd \${AP_SC28_VAGRANT_VMS_DIR}/vg-nextcloud; vagrant ssh -c /home/vagrant/pnphuong29/projects/p29-github/pnphuong29/ap-scripts-nextcloud-sc56/utils/ap_rt_nextcloud_db.sh"
...
ap_func_nextcloud_configure_php() {
    ...
    ap_pos=$(cat "${ap_php_config_dir}/apache2/php.ini" | grep -n "${AP_MARKER_START}" | head -1 | cut -d : -f 1)
    ...
    ap_pos=$(cat "${ap_php_config_dir}/cli/php.ini" | grep -n "${AP_MARKER_START}" | head -1 | cut -d : -f 1)
...
ap_func_nextcloud_install() {
    ...
    if [[ -z "${AP_NEXTCLOUD_SERVER_DB_PASSWD}" ]]; then
        echo "DB password cannot be empty!"
        ap_alias_ret_error_missing_info
    fi
